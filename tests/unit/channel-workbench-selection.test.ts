...
describe('channel workbench selection', () => {
  it('keeps the same channel family when the active channel id becomes stale', () => {
    const channels = [
      ...
      createChannel({ id: 'wechat-e5e00d1a769e-im-bot', type: 'wechat', name: '寰俊' }),
    ];
  ...
  it('still honors the requested channel when there is no active channel id', () => {
    const channels = [
      createChannel({ id: 'feishu-default', type: 'feishu', name: 'Feishu' }),
      createChannel({ id: 'wechat-default', type: 'wechat', name: '寰俊' }),
    ];
  ...
  it('stays neutral when neither the route nor the store selected a channel', () => {
    const channels = [
      createChannel({ id: 'feishu-default', type: 'feishu', name: 'Feishu' }),
      createChannel({ id: 'wechat-default', type: 'wechat', name: '寰俊' }),
    ];

    const selected = resolveSelectedChannel(channels, null, null);
