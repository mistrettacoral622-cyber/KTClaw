...
describe('ContextRail', () => {
  ...
  it('renders expanded cards by default', () => {
    ...
    expect(screen.getByText(FALLBACK_WORKBENCH_DATA.team.name)).toBeInTheDocument();
    expect(screen.getByText(FALLBACK_WORKBENCH_DATA.channel.name)).toBeInTheDocument();
    expect(screen.getByText(FALLBACK_WORKBENCH_DATA.task.title)).toBeInTheDocument();
  });
  ...
  it('renders collapsed handle and expands when clicked', () => {
    ...
    expect(screen.queryByText('上下文 Context')).not.toBeInTheDocument();
