...
サイドバー検索は検索ボタンまたは `Ctrl/Cmd+K` から開けるようになり、会話、エージェント、ページ、さらにキャッシュ済みのチャット履歴本文まで検索できます。会話はサイドバーでピン留めでき、会話とエージェントの右クリックメニュー、`/export`、さらにチャット画面の `会話` ドロワーから Markdown エクスポートや管理操作を行えます。Activity ページも、生ログ表示からフィルタ可能な構造化イベントカード + raw line 展開 + live 自動更新へ更新されました。
